...
public class SimpleSliderLayout extends RelativeLayout {
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        simpleViewPager = parent.findViewById(R.id.simple_slider_viewpager);
    }
    ...
    public void setCycling(boolean isCycling) {
        ...
        if (this.isCycling && infinitePagerAdapter.getRealCount() > 2) {
            cycling();
        } else {
            stopCycling();
        }
    ...
    private void stopCycling() {
        simpleViewPager.setAdapter(infinitePagerAdapter.getRealAdapter());
    }
    ...
    public void moveNextPosition(boolean smooth) {
        ...
        if (getRealAdapter() == null)
            throw new IllegalStateException("You did not set a slider adapter");
        if (getRealAdapter().getCount() <= 2 || (!isCycling && simpleViewPager.getCurrentItem() == getRealAdapter().getCount() - 1))
